...
def train(model, optimizer, conf_dict, train_generator, torch_scaler):
    ...
    for X_batch, y_batch, phonemes in train_generator:
        optimizer.zero_grad()

        # Move to GPU
        X_batch = torch_scaler.transform((torch.from_numpy(X_batch)).to(device))
        y_batch = (torch.from_numpy(y_batch)).to(device)
        phonemes = (torch.from_numpy(phonemes)).to(device)

        # Get outputs
        train_outputs = model(X_batch, phonemes.float()) # assuming phoneme is known, just input it to training
...
def validate(model, conf_dict, valid_generator, torch_scaler):
    ...
    with torch.no_grad():
        for X_val, y_val, phonemes in valid_generator:
            # Move to GPU
            ...
            outputs = model(X_val, phonemes.float())
...
def train_and_validate(conf_file, model_name):
    ...
    if os.path.exists(model_dir):
        print(f"Directory '{model_dir}' already exists. Exiting program.")
        exit()
    else:
    ...
    for epoch in iterator:
        with open(training_curves, "a") as file_obj:
            ...
            logging.info("Time: {}, Epoch: {}, Loss: {}".format(current_time, epoch+1, loss[-1]))
