...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not np.all(phones == 'h#'):
            ...
            if bool(self.le):
                phones_encoded = encode_phones(phones,self.le)

            # Create chunks
            X = create_chunks(X, self.conf_dict)
            y = create_chunks(y, self.conf_dict)
            phones = create_chunks(phones_encoded, self.conf_dict)
        ...
        # Exclude feature files where forced alignment was not successful
            ...
            phones = np.empty(shape=(0, self.conf_dict["chunk_len"], len(self.le.classes_)), dtype=np.float32)
...
def create_chunks_phones(p, conf_dict):
    """
    Same as create_chunks, but for phones (except if phones are encoded)
...
def encode_phones(phones,le):
    '''
    inputs:
        phones (np.array): 1D matrix of phonemes e.g. np.array(["sil","aa"])
        le (sklearn.preprocessing.LabelEncoder): 
    '''
    phones_encoded = np.zeros((phones.shape[0], len(le.classes_)), dtype="int32")
    phoneme_to_indx = {}
    le_class_numbers = le.fit_transform(le.classes_)
    for indx in range(len(le.classes_)):
        phoneme_to_indx[le.classes_[indx]] = le_class_numbers[indx]
    phones_encoded = np.zeros((phones.shape[0], len(le.classes_)), dtype="int32")
    for indx in range(len(phones)):
        phoneme = phones[indx]
        # print(indx, phoneme)
        indx_to_change = phoneme_to_indx[phoneme]
        phones_encoded[indx][indx_to_change] = 1
    return phones_encoded